...
if run_simulations == 'y':
    ...
    B_days = np.linspace(-0.6,-1.0,9, endpoint=True)
    
    B_days = np.delete(B_days,0)
    ...
    # data.save('steady-state_Bdot_' + "{:.2f}".format(B_days[0]) + '_hybr.pickle')
    
    
    for j in np.arange(0,len(B)):
        file = open('steady-state_Bdot_-0.60_21pts.pickle','rb')
        data = pickle.load(file)
        file.close()
        
        print('melt rate: ' + "{:0.2f}".format(B_days[j]) + ' m/d')
        #data = glaciome(n_pts, dt, L, Ut, Uc, Ht, X_fjord, W_fjord)
        data.B = B[j]
        ...
        data.save('steady-state_Bdot_' + "{:.2f}".format(B_days[j]) + '_21pts.pickle')


#%%
files = sorted(glob.glob('*21pts.pickle'))
#files = files[10:]
#files = files[::5]
...
ax2.set_xticks(np.linspace(0.5,1.0,6,endpoint=True))
...
plt.savefig('fig_steady-state_melt_rates.pdf',format='pdf',dpi=300)
#plt.close()
#%%